...
import {
  ...
  type WorkoutExerciseCardViewModel,
} from "./workout-exercise-card.view-model";
import type { WorkoutExerciseGroup, Exercise, WorkoutSet } from "~/modules/fitness/domain/workout";
...
describe("WorkoutExerciseCardViewModel", () => {
  ...
  describe("createWorkoutExerciseCardViewModel", () => {
    it("should create view model with correct basic properties", () => {
      const viewModel = createWorkoutExerciseCardViewModel(mockExerciseGroup, false);
    ...
    it("should return empty total volume when no completed sets", () => {
      const setsWithoutCompleted = mockSets.map(set => ({ ...set, isCompleted: false }));
      const exerciseGroup = { ...mockExerciseGroup, sets: setsWithoutCompleted };
      
      const viewModel = createWorkoutExerciseCardViewModel(exerciseGroup, false);
    ...
    it("should set correct permissions when workout is complete", () => {
      const viewModel = createWorkoutExerciseCardViewModel(mockExerciseGroup, true);
