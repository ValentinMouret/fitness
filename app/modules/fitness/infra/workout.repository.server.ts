        ...
        ? [
            {
              ...
              sets: sets
                ...
                .map((set) => ({
                  ...
                  targetRirMin: set.targetRirMin ?? undefined,
                  targetRirMax: set.targetRirMax ?? undefined,
                  targetRirSource: set.targetRirSource ?? undefined,
                  reportedRir: set.reportedRir ?? undefined,
  ...
  for (const group of exerciseGroups) {
    ...
    if (changedSets.length) {
      const values = changedSets.map((set) => ({
        ...
        targetRirSource: set.targetRirSource ?? null,
        reportedRir: set.reportedRir ?? null,
        isCompleted: set.isCompleted,
        isWarmup: set.isWarmup,
        isFailure: set.isFailure,
      }));
      await tx
        ...
        .onConflictDoUpdate({
          ...
          set: {
            ...
            targetRirMin: sql`excluded.target_rir_min`,
            targetRirMax: sql`excluded.target_rir_max`,
            targetRirSource: sql`excluded.target_rir_source`,
            reportedRir: sql`excluded.reported_rir`,
        ...
        sets: [
          {
            ...
            rpe: workoutSet.rpe,
            targetRirMin: workoutSet.targetRirMin,
            targetRirMax: workoutSet.targetRirMax,
            targetRirSource: workoutSet.targetRirSource,
...
function workoutSetRecordToDomain(
  ...
  return {
    ...
    targetRirMin: record.targetRirMin ?? undefined,
    targetRirMax: record.targetRirMax ?? undefined,
    targetRirSource: record.targetRirSource ?? undefined,
    reportedRir: record.reportedRir ?? undefined,
  };
}
